...
BTN = {
    ...
    "model_dental": "Модель: общая (быстро)",
    "model_tf2": "Модель: по зубам (ToothFairy2)",
...
TIP = {
    ...
    "model_pick": "Общая: быстро, зубы единым блоком. По зубам (ToothFairy2): "
                  "каждый зуб отдельно (FDI), но дольше и требует больше памяти.",
}
...
MSG = {
    ...
    "engine_update_banner": (
        "⚠ Установлен устаревший движок сегментации. Без обновления не работает "
        "индикатор прогресса «Сегментация… %» и возможны старые ошибки. "
        "Обновите движок →"
    ),
