...
**Последнее обновление: 2026-06-13 (вечер).** Всё закоммичено и запушено в `origin/master`.
Ветка: `master`. **Актуальный РЕЛИЗ — v0.3.1.** Тесты: `.venv/bin/python -m pytest -q` → **219 passed**.
